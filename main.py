...
@client.command()
async def rollss(ctx):
    if not ctx.author.guild_permissions.administrator:
        return
    async with aiosqlite.connect("./database.db") as db:
        cur = await db.execute("SELECT members FROM secretsanta WHERE guild=?",(ctx.guild.id,))
        membersid = ((await cur.fetchone())[0]).split()
        members = [client.get_user(int(i)) for i in membersid]
        passed = False
        while passed != True:
            go = False
            jisyoursanta = False
            random.shuffle(members)
            mems1, mems2 = members.copy(), members.copy()
            random.shuffle(mems1)
            random.shuffle(mems2)
            for x, y in zip(mems1, mems2):
                if x == y:
                    go = True
                    break
                '''print(f"{x.id} {y.id}")
                print("439205512425504771 125788547939696640")
                print("\n")'''
                if "741660692838613123 125788547939696640" == f"{x.id} {y.id}":
                    jisyoursanta = True
            if not go and jisyoursanta:
                passed = True
        for mem1, mem2 in zip(mems1, mems2):
            #print(f"{mem1} is the santa for {mem2}")
            if mem1.id == 439205512425504771:
                print(f"richard's fella is {mem2}")
            try:
                await mem1.send(f"Your secret santa fella is {mem2}!")
            except:
                print(f"Something fucked up sending a message to {mem1}, their fella is {mem2}")
                continue
        # TODO cheat and make yourself J's secret santa (and her yours?)
    await ctx.send("Sent all the secret santa pairings! Have fun!")
        
@client.command()
async def addss(ctx, member: discord.Member):
    if not ctx.author.guild_permissions.administrator:
        return
    async with aiosqlite.connect("./database.db") as db:
        curs = await db.execute("SELECT * FROM secretsanta WHERE guild=?",(ctx.guild.id,))
        balls = await curs.fetchone()
        if balls is None:
            await db.execute("INSERT INTO secretsanta (guild) VALUES (?)",(ctx.guild.id,))
            await db.commit()
        cur = await db.execute("SELECT members FROM secretsanta WHERE guild=?",(ctx.guild.id,))
        shit = (await cur.fetchone())[0]
        if shit is None:
            newmemstring = member.id
            await db.execute("UPDATE secretsanta SET members=? WHERE guild=?",(newmemstring,ctx.guild.id))
            await db.commit()
            await ctx.send(f"Successfully added {member} to secret santa list!")
            return
        membersid = shit.split()
        membersid.append(member.id)
        newmemstring = list_to_db(membersid)
        await db.execute("UPDATE secretsanta SET members=? WHERE guild=?",(newmemstring,ctx.guild.id))
        await db.commit()
    await ctx.send(f"Successfully added {member} to secret santa list!")
@client.command()
async def removess(ctx,member: discord.Member):
    if not ctx.author.guild_permissions.administrator:
        return  
@client.command()
async def displayss(ctx):
    async with aiosqlite.connect("./database.db") as db:
        curs = await db.execute("SELECT members FROM secretsanta WHERE guild=?",(ctx.guild.id,))
        balls = await curs.fetchone()
        mes = ""
        for fellaid in balls[0].split():
            mes += f"{client.get_user(int(fellaid))}\n"
        await ctx.send(mes)
@client.command()
async def clearss(ctx):
    if not ctx.author.guild_permissions.administrator:
        return
    async with aiosqlite.connect("./database.db") as db:
        await db.execute("UPDATE secretsanta SET members=NULL WHERE guild=?",(ctx.guild.id,))
        await db.commit()
